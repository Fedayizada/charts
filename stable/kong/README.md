...
If using either RBAC or the Portal, create a Secret with two keys (`admin` and
`portal` containing your plugin configuration, e.g.
...
$ kubectl create secret generic kong-session-config --from-file=admin --from-file=portal
...
`.enterprise.session_conf_secret` key.
